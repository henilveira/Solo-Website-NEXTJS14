...
const AdminProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  const { listarUsuarios } = useCompany();  // Verifica autenticação e status de admin
  ...
  useEffect(() => {
    const verifySoloAuth = async () => {
      try {
        const isAuthorized = await listarUsuarios();
    ...
    verifySoloAuth();
  }, [checkAuth, listarUsuarios, router, isRedirecting]);
